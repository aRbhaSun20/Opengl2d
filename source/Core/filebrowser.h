...
private:
    ...
    bool IsExtensionMatched(const fs::path &extension) const;
...
inline ImGui::FileBrowser::FileBrowser(ImGuiFileBrowserFlags flags)
    : width_(700), height_(450), flags_(flags),
      ...
      inputNameBuf_(std::array<char, INPUT_NAME_BUF_SIZE>())
{
    if (flags_ & ImGuiFileBrowserFlags_CreateNewDir)
        newDirNameBuf_ = std::array<char, INPUT_NAME_BUF_SIZE>();
...
inline bool ImGui::FileBrowser::IsExtensionMatched(
    const fs::path &_extension) const
{
#ifdef _WIN32
    fs::path extension = ToLower(_extension.string());
#else
